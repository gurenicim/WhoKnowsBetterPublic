        ...
        List {
            ForEach(aList, id: \.self) { usr in
                HStack {
                    ...
                    Spacer()
                    Label("", systemImage: "xmark.circle").labelStyle(.iconOnly).onTapGesture {
                        player.declineFriendRequest(friendUsername: usr)
                        aList.remove(at: aList.firstIndex(of: usr)!)
                    }
                    Label("", systemImage: "checkmark.circle").labelStyle(.iconOnly).onTapGesture {
                        player.acceptFriendRequest(friendUsername: usr)
                        aList.remove(at: aList.firstIndex(of: usr)!)
                    }
                }
